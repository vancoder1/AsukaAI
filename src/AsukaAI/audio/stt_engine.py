...
if __name__ == "__main__":
    # Configure basic logging if custom config failed earlier
    if 'lc' not in globals():
        logging.basicConfig(level=logging.INFO, # Use INFO for general use, DEBUG for detailed logs
                            format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        logger = logging.getLogger(__name__) # Ensure logger exists

    stt_instance = None
    try:
        stt_instance = PushToTalkSTT()
        print("\n--- Push-to-Talk STT Ready ---")
        print(f"Press and hold the '{PTT_KEY}' key to record.")
        print("Release the key to transcribe.")
        print("Press Ctrl+C to exit.")
        print("--------------------------------\n")

        while True:
            # Blocking call that waits for one full PTT cycle
            transcription = stt_instance.get_transcription(timeout=None) # Wait indefinitely

            if transcription: # Avoid printing empty lines if no audio was recorded
                print("\n--- Transcription ---")
                print(f"{transcription}")
                print("---------------------\n")
            else:
                print("(No audio detected or transcription empty)")

            # Small delay before waiting again, purely optional
            # time.sleep(0.1)

    except RuntimeError as e:
        # Catch initialization errors (model loading, hotkeys)
        logger.critical(f"Runtime Error during STT setup or operation: {e}")
        print(f"\nFATAL ERROR: {e}. Check logs for details.", file=sys.stderr)
    except KeyboardInterrupt:
        print("\nCtrl+C detected. Exiting...")
    except Exception as e:
        # Catch unexpected errors in the main loop
        logger.error(f"An unexpected error occurred in the main loop: {e}", exc_info=True)
        print(f"\nAn unexpected error occurred. Check logs.", file=sys.stderr)
    finally:
        if stt_instance:
            stt_instance.close()
